...
class CallableChalkee {
    ...
    private createStateUpdater(modifierFn: (instance: CallableChalkee) => void): any {
        ...
        // Set the parent to self so that when the function wrapper captures self,
        // it captures the correct instance with the updated state
        const newInstance = new CallableChalkee(self, ['', self.getState()])
